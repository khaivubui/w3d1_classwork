...
def andrews_films_and_leads
  ...
  # Andrews' played in.
  execute(<<-SQL)
    SELECT movies_with_Julie_Andrews.title, actors.name
    FROM (
    ...
    JOIN castings ON movies_with_Julie_Andrews.id = movie_id
    JOIN actors ON actors.id = actor_id
    WHERE castings.ord = 1
  SQL
end
...
def colleagues_of_garfunkel
  ...
  execute(<<-SQL)
    ...
    ) AS movies_with_art_garfunkel
      ON movies_with_art_garfunkel.id = movie_id
